...
static char rcsid[] UTIL_UNUSED = "$Id: cmdMisc.c,v 1.12.2.1.4.9.4.2 2007/05/21 07:26:19 nusmv Exp $";
  ...
  char ** argv)
{
  ...
  do {
    ...
    do {
	  ...
	  const char* promp_sym = " > ";
	  char* stmp = ALLOC(char, strlen(get_pgm_name(options))+
			     strlen(promp_sym)+1);
	  nusmv_assert(stmp != (char*) NULL);

	  stmp[0] = '\0';
	  strcat(stmp, get_pgm_name(options));
	  strcat(stmp, promp_sym);

          prompt_string = stmp;
        }
	else {
	  prompt_string = strsav(prompt_string);
        }
      }
      else {
