...
%left <lineno> TOK_VAR TOK_IVAR TOK_DEFINE TOK_INIT TOK_TRANS TOK_INVAR TOK_SPEC TOK_CTLSPEC TOK_CTLGSPEC TOK_LTLSPEC TOK_COMPUTE 
...
%left <lineno> TOK_INCONTEXT TOK_SIMPWFF TOK_LTLWFF TOK_LTLPSL TOK_CTLWFF TOK_COMPWFF TOK_CTLGWFF
...
%type <node> constant primary_expr primg_expr case_element_expr case_element_list_expr
%type <node> concatination_expr multiplicative_expr
%type <node> additive_expr remainder_expr shift_expr
%type <node> set_expr setg_expr set_list_expr setg_list_expr union_expr uniong_expr in_expr ing_expr relationalg_expr relational_expr 
...
%type <node> simple_expression next_expression ctl_expression ltl_expression 
 ...
 /********************************************************/
%type <node> ctlg_expr pure_ctlg_expr ctlg_basic_expr
 //%type <node> ctlg_or_expr ctlg_iff_expr ctlg_implies_expr ctlg_and_expr 
...
%type <node> invarspec ctlspec ctlgspec ltlspec pslspec constants constants_expression
...
setg_expr    : subrange
	     | primg_expr
             | TOK_LCB setg_list_expr TOK_RCB   { $$ = $2; }
             ;

setg_list_expr : ctlg_basic_expr
               | setg_list_expr TOK_COMMA ctlg_basic_expr {$$ = new_lined_node(UNION, $1, $3, $2);}
               ;


uniong_expr  : setg_expr
             | uniong_expr TOK_UNION setg_expr { $$ = new_lined_node(UNION, $1, $3, $2); }
             ;

ing_expr :   uniong_expr
             | ing_expr TOK_SETIN uniong_expr { $$ = new_lined_node(SETIN, $1, $3, $2); }
             ;

primg_expr :
               constant
             | TOK_MINUS primg_expr { $$ = new_lined_node(UMINUS, $2, Nil, $1); }
             | TOK_ATOM
	     | TOK_LP ctlg_basic_expr TOK_RP             { $$ = $2; }
             | TOK_NOT primg_expr   { $$ = new_lined_node(NOT, $2, Nil, $1); }
	     | primg_expr TOK_DOT TOK_ATOM
                    {
		      if (ATOM != node_get_type($1) && DOT != node_get_type($1) && ARRAY != node_get_type($1) && SELF != node_get_type($1)) {
			yyerror_lined("incorrect DOT expression", $2);
		        YYABORT;
	      ...
	      | primg_expr TOK_DOT TOK_NUMBER
		    ...
		    YYABORT;
             ...
             | primg_expr TOK_LB ctlg_basic_expr TOK_RB
                     { 
		       if (ATOM != node_get_type($1) && DOT != node_get_type($1) && ARRAY != node_get_type($1) && SELF != node_get_type($1)) {
			yyerror_lined("incorrect ARRAY expression", $2);
			YYABORT;
             ...
             | primg_expr TOK_LB ctlg_basic_expr TOK_COLON ctlg_basic_expr TOK_RB
                       { 
			$$ = new_lined_node(BIT_SELECTION, $1, 
			               new_lined_node(COLON, $3, $5, $4), $2);
		       }
...
relationalg_expr 
	     :ing_expr
             //| primg_expr
	     /*| TOK_LP relationalg_expr TOK_RP { $$ = $2; }*/
	     | relationalg_expr TOK_EQUAL primg_expr { $$ = new_lined_node(EQUAL, $1, $3, $2); }
             | relationalg_expr TOK_NOTEQUAL primg_expr { $$ = new_lined_node(NOTEQUAL, $1, $3, $2); }
             | relationalg_expr TOK_LT primg_expr { $$ = new_lined_node(LT, $1, $3, $2); }
             | relationalg_expr TOK_GT primg_expr { $$ = new_lined_node(GT, $1, $3, $2); }
             | relationalg_expr TOK_LE primg_expr { $$ = new_lined_node(LE, $1, $3, $2); }
             | relationalg_expr TOK_GE primg_expr { $$ = new_lined_node(GE, $1, $3, $2); }
             ;

ctlg_expr     : relationalg_expr 
              | TOK_LP ctlg_expr TOK_RP { $$ = $2; }
              | pure_ctlg_expr /* all CTL operators */
              ;

pure_ctlg_expr 
             : TOK_EX ctlg_basic_expr       { $$ = new_lined_node(EGX, new_lined_node(EX, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
	     | TOK_EX number ctlg_basic_expr       { $$ = new_lined_node(EGX, new_lined_node(EX, $3, Nil, $1), $2, $1);}
	     | TOK_AX ctlg_basic_expr       { $$ = new_lined_node(AGX, new_lined_node(AX, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
	     | TOK_AX number ctlg_basic_expr       { $$ = new_lined_node(AGX, new_lined_node(AX, $3, Nil, $1), $2, $1);}
	     | TOK_AG ctlg_basic_expr       { $$ = new_lined_node(AGG, new_lined_node(AG, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_AG number ctlg_basic_expr       { $$ = new_lined_node(AGG, new_lined_node(AG, $3, Nil, $1), $2, $1);}
	     | TOK_EF ctlg_basic_expr       { $$ = new_lined_node(EGF, new_lined_node(EF, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_EF number ctlg_basic_expr       { $$ = new_lined_node(EGF, new_lined_node(EF, $3, Nil, $1), $2, $1);}
	     | TOK_AF ctlg_basic_expr       { $$ = new_lined_node(AGF, new_lined_node(AF, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_AF number ctlg_basic_expr       { $$ = new_lined_node(AGF, new_lined_node(AF, $3, Nil, $1), $2, $1);}

             | TOK_AA TOK_LB ctlg_basic_expr TOK_UNTIL ctlg_basic_expr TOK_RB
                                     { $$ = new_lined_node(AGU, new_lined_node(AU, $3, $5, $1), 
							   new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_AA number TOK_LB ctlg_basic_expr TOK_UNTIL ctlg_basic_expr TOK_RB
                                     { $$ = new_lined_node(AGU, new_lined_node(AU, $4, $6, $1), $2, $1);}
	     | TOK_EG ctlg_basic_expr       { $$ = new_lined_node(EGG, new_lined_node(EG, $2, Nil, $1), 
							    new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_EG number ctlg_basic_expr       { $$ = new_lined_node(EGG, new_lined_node(EG, $3, Nil, $1), $2, $1);}
             | TOK_EE TOK_LB ctlg_basic_expr TOK_UNTIL ctlg_basic_expr TOK_RB
                                     { $$ = new_lined_node(EGU, new_lined_node(EU, $3, $5, $1), 
							   new_node(NUMBER, NODE_FROM_INT(0), Nil), $1);}
             | TOK_EE number TOK_LB ctlg_basic_expr TOK_UNTIL ctlg_basic_expr TOK_RB
                                     { $$ = new_lined_node(EGU, new_lined_node(EU, $4, $6, $1), $2, $1);}

             /* NOT is required here to allow such expr as "! EX a" */
             | TOK_NOT pure_ctlg_expr { $$ = new_lined_node(NOT, $2, Nil, $1); }
             ; 
ctlg_basic_expr : 
                  ctlg_expr
                | ctlg_basic_expr TOK_IMPLIES ctlg_basic_expr { $$ = new_lined_node(IMPLIES, $1, $3, $2); }
	        | ctlg_basic_expr TOK_IFF ctlg_basic_expr   { $$ = new_lined_node(IFF, $1, $3, $2); }
                | ctlg_basic_expr TOK_OR ctlg_basic_expr    { $$ = new_lined_node(OR,$1, $3, $2); }
                | ctlg_basic_expr TOK_XOR ctlg_basic_expr   { $$ = new_lined_node(XOR,$1, $3, $2); }
                | ctlg_basic_expr TOK_XNOR ctlg_basic_expr  { $$ = new_lined_node(XNOR,$1, $3, $2); }
	        | ctlg_basic_expr TOK_AND ctlg_basic_expr  { $$ = new_lined_node(AND, $1, $3, $2); }
		;
...
{ 
  ...
  $$ = new_lined_node(PSLSPEC, psl_parsed_tree, Nil, $1); 
              ...
              ;
/****************************/
ctlgspec      : TOK_CTLGSPEC ctlg_basic_expr optsemi {$$ = new_lined_node(CTLGSPEC, $2, Nil, $1);}
              ;
...
command_case  : TOK_GOTO state TOK_SEMI
	      ...
	      | TOK_CTLGWFF ctlg_basic_expr TOK_SEMI
                 {$$ = new_lined_node(CTLGWFF,
                             new_node(CONTEXT, Nil, $2), Nil, $1);
                 }
	      | TOK_CTLGWFF ctlg_basic_expr TOK_INCONTEXT context TOK_SEMI
		 {$$ = new_lined_node(CTLGWFF,
		            new_node(CONTEXT, context2maincontext($4), $2), Nil, $1);
                 }
